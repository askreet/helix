            ...
            .map(|path| {
                (
                    ...
                    Some((
                        ...
                        self.end_line.unwrap_or(self.line).saturating_sub(1),
                    )),
                )
            })
    }
...
impl ui::Locate for Thread {
    fn locate(&self, editor: &Editor) -> Option<ui::FileLocation> {
        let frames = editor.debugger.as_ref()?.stack_frames.get(&self.id)?;
